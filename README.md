...
2. **Micro-Rollouts & Snap-Back:** During training, the agent executes an $N=3$ micro-rollout using uniform time sampling $t \sim \mathcal{U}(0, 1)$ and linear interpolation. The environment then executes a "snap-back" re-alignment mechanism to the true `dopri5` manifold, preventing compounding mathematical drift from corrupting the Q-value estimations.
